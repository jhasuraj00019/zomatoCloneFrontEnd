...
class Pagetop extends React.Component {
  ...
  componentDidMount() {
    if (
      ...
      //   console.log("cartFirstPage", cart);
      // }
  ...
  handleLocationChange = (event) => {
    ...
    axios({
      method: "GET",
      url: `https://zomato-clone-backend2.herokuapp.com/restaurantbylocation/${locationId}`,
    ...
    if (suggestions.length === 0 && searchText) {
      return (
        <ul className="suggestionList">
          <li className="suggestions">No Match Found</li>
        </ul>
      );
    } else {
      return (
  ...
  //Account Creating Detail is put into state
  ...
  makeAccount = (event) => {
    ...
    axios({
      method: "POST",
      url: "https://zomato-clone-backend2.herokuapp.com/signup",
      headers: { "Content-Type": "application/json" },
  ...
  checkCredentials = () => {
    ...
    axios({
      method: "GET",
      url: `https://zomato-clone-backend2.herokuapp.com/getUserByEmail/${loginInfo.email}`,
      headers: { "Content-Type": "application/json" },
    })
      .then((response) => {
  ...
  uploadPicture = (event) => {
    ...
    // profilePic = event.target.files[0] ? event.target.files[0] : undefined;
    let img = new Image();
    img.onload = () => {
      profilePic = img.result;
    };
    // if (file) {
      ...
      <React.Fragment>
        <div className="container">
          <div className="displayimage">
            <div className="account account2">
              {isLoggedIn ? (
                <>
                  {/* <span className="profilPhoto"></span> */}
                  {/* <span
                    className="cartContainer"
                    onClick={() => this.modal("orderModal", true)}
                  >
                  ...
                  <span id="login">
                    <div className="userOptions">
                      {/* <img
                        ...
                        src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRlWdoN08QdBV52ggwrK5zkE0wJ5MgUNOl8w&usqp=CAU"
                      /> */}
            ...
            <div className="upper">
              ...
              <p id="discription">Find the best restaurants, cafes, and bars</p>
              <div>
                <select
                  className="selection"
                  name="city"
        ...
        <div>
          ...
          <Modal isOpen={signupFormOpens} style={customStyles}>
            ...
            <div>
              <form
                ...
                onSubmit={() => {
                  this.makeAccount();
                  this.modal("signupFormOpens", false);
                }}
              >
                <div id="imgUpload">
                  ...
                  <input
                    ...
                    name="userImg"
                    accept="image/*"
                    onChange={(event) => this.uploadPicture(event)}
                    hidden
                  />
                </div>
          ...
          {/* <Modal isOpen={orderModal} style={customStyles}>
            <div className="itemContainer">
              <button
                className="closeModal"
                onClick={() => this.modal("orderModal", false)}
              >
                &#10006;
              </button>
